...
export default function Overview() {
  ...
  return <Shell><div className="page">
    <div className="page-intro"><div><div className="eyebrow accent">MATCH CENTER</div><h1>Fixture explorer</h1><p>Signals, prices, and context for the global cricket slate.</p></div><button data-testid="refresh-fixtures-button" className="outline-btn" onClick={refresh} disabled={loading}><RefreshCw size={15}/> {loading ? "REFRESHING…" : "REFRESH FEED"}</button></div>
    ...
    <div className="metrics"><Metric label="Upcoming fixtures" value={total || "—"} delta="Live provider feed" icon={Activity}/><Metric label="Model coverage" value="—" delta="Reported per fixture" icon={Database}/><Metric label="Mean confidence" value="—" delta="Calculated from loaded data" icon={TrendingUp}/><Metric label="Pipeline status" value={error ? "DEGRADED" : "NOMINAL"} delta={error ? "API request failed" : "API reachable"} icon={Gauge}/></div>
    ...
    {error && <div className="error" role="alert" data-testid="fixture-error">{error}</div>}
