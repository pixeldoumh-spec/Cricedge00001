...
import { Activity, ArrowUpRight, ChevronRight, CircleHelp, Database, Gauge, Layers3, RefreshCw, ShieldCheck, Target, TrendingUp } from "lucide-react";
import "@/App.css";

const API = `${process.env.REACT_APP_BACKEND_URL}/api`;
const fmt = (value) => new Date(value).toLocaleString([], { weekday: "short", hour: "2-digit", minute: "2-digit" });
const nav = [{to:"/", label:"Fixtures", icon:Activity}, {to:"/portfolio", label:"Portfolios", icon:Layers3}];
  ...
  return <div className="markets-board" data-testid="markets-board">
    {shown.map(group => <div className="market-group" data-testid={`market-group-${group.toLowerCase().replace(/ /g,'-')}`} key={group}>
      ...
      {groups[group].map(market => <div className="market-row" key={market.key} data-testid={`market-${market.key}`}>
        <div className="market-label"><b>{market.label}{market.source==="LIVE" && <span className="live-badge" data-testid={`live-badge-${market.key}`}>LIVE</span>}</b><small>{market.line}</small></div>
...
function App(){return <BrowserRouter><Routes><Route path="/" element={<Overview/>}/><Route path="/fixture/:id" element={<Detail/>}/><Route path="/portfolio" element={<Portfolio/>}/></Routes></BrowserRouter>}
export default App;