...
class FeatureEngine:
    ...
    def features_for(self, team_a: str, team_b: str) -> Dict[str, float]:
        ...
        # Strength is the signed mean difference of the six paired metrics;
        # therefore it is derived solely from the preceding 12 feature values.
    ...
    def update(self, match: Dict[str, Any]) -> None:
        ...
        if len(teams) != 2:
            raise ValueError("ODI O0 requires exactly two teams")
        ...
        for inn in innings:
            ...
            r0, b0 = batting[batting_team]
            batting[batting_team] = (r0 + runs, b0 + balls)
            r0, b0, w0 = bowling[opponent]
            bowling[opponent] = (r0 + runs, b0 + balls, w0 + wickets)
        ...
        for team in teams:
            ...
            if winner in teams:
                ...
                s.wins += int(winner == team)
                if team == second_team:
                    s.chase_decisive += 1
                    s.chase_wins += int(winner == team)
                elif team == first_team:
                    s.defend_decisive += 1
                    s.defend_wins += int(winner == team)
