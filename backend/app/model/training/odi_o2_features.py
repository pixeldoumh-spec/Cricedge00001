...
class O2FeatureEngine(O0FeatureEngine):
    """O0 plus one leakage-safe ODI wicket-preservation feature."""

    def __init__(self) -> None:
        super().__init__()
        self._innings_count: Dict[str, int] = defaultdict(int)
        self._wickets_lost: Dict[str, int] = defaultdict(int)

    @staticmethod
    def _preservation(innings: int, wickets_lost: int) -> float:
        return float((10 * innings - wickets_lost) / (10 * innings)) if innings else 0.0

    def features_for(self, team_a: str, team_b: str) -> Dict[str, float]:
        values = super().features_for(team_a, team_b)
        a = self._preservation(self._innings_count[team_a], self._wickets_lost[team_a])
        b = self._preservation(self._innings_count[team_b], self._wickets_lost[team_b])
        values[O2_FEATURE_NAME] = a - b
        return values
                ...
                for delivery in over.get("deliveries", []):
                    for wicket in delivery.get("wickets", []):
                        if wicket.get("kind") not in {"retired hurt", "retired not out"}:
                            wickets += 1
            self._innings_count[team] += 1
            self._wickets_lost[team] += min(wickets, 10)
        super().update(match)
